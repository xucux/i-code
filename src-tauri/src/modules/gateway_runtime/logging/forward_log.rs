//! # 转发日志记录器
//!
//! 将 `LogRecord` 写入自研 logger 模块的环形缓冲区（`LogSource::ProviderApi`），
//! 按转发日志开关与最大 body 长度配置截断请求/响应体。
//!
//! 不处理 tauri-plugin-log 输出（由 `TauriLogEmitter` 负责）。
...
/// 转发日志记录器
///
/// 写入自研 logger，按 `forward_*` 系列开关控制是否记录请求/响应体，
/// 并按 `forward_max_body_length` 截断。
pub struct ForwardLogRecorder;

impl LogRecorder for ForwardLogRecorder {
    fn record(&self, shared: &GatewaySharedState, entry: &LogRecord) {
        // 仅处理转发日志
        if !matches!(entry.kind, Some(LogKind::Forward)) {
            return;
        }

        let settings = shared.logger_handle.service().get_settings();
        let fwd_cfg = settings.to_forward_config();
        drop(settings);

        let request_body = entry
            .request_body
            .as_deref()
            .filter(|_| fwd_cfg.enable_request_log)
            .map(|s| truncate_body(s, fwd_cfg.max_body_length));

        let response_body = entry
            .response_body
            .as_deref()
            .filter(|_| fwd_cfg.enable_response_log)
            .map(|s| truncate_body(s, fwd_cfg.max_body_length));
