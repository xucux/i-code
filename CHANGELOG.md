...
> [!IMPORTANT]
> 数据库结构变更：调用记录表新增幂等累加标记列，并自动从明细记录重建统计聚合表，升级后首次启动自动完成迁移。

### 🐞修复

- **模型调用统计聚合与明细不一致**：修复流式请求被完成两次（估算 Token 收尾 + 真实 usage 收尾）导致同一调用在预聚合统计中被重复累加的问题；升级后聚合表将从明细记录全量重建，保证「聚合汇总」与「明细汇总」口径一致
...
- **更新弹窗下载中可关闭**：更新包下载期间允许关闭弹窗，关闭时提供两种选择——「关闭并继续下载」（后台继续下载，完成后 toast 提醒）或「关闭并停止下载」（中止下载并清理临时文件）

- **Windows 安装器多语言**：NSIS 安装界面支持简体中文/英文，按系统语言自动选择（不弹语言选择框，简体中文系统默认中文安装界面）

## [0.3.3] - 2026-09-03

> [!IMPORTANT]
> 数据库结构变更：新增视觉生成供应商标识与图像生成历史、视频任务两张表，升级后首次启动自动完成迁移。

### 🚀新增

- **视觉生成页（文生图）**：新增 `/vision` 页面，提供参数工作台与结果画廊双 Tab——工作台支持选择视觉生成供应商 / 模型、提示词输入（含右键复制、粘贴系统剪贴板）、尺寸（2K 分辨率 11 种比例，如 16:9、21:9、1:1 等）、数量与水印开关；画布实时预览生成的图片，一键下载 / 复制到剪贴板；画廊展示全部生成历史，支持灯箱放大与删除

- **媒体产物本地化存储**：生成成功后立即将供应商返回的临时图片下载到应用数据目录（供应商 URL 存在过期机制，如 SenseNova 固定 1 小时），数据库仅存相对路径，画廊缩略图与灯箱共享 Base64 缓存

- **内置视觉生成预设**：内置「Sensenova Image」图像生成供应商（OpenAI 兼容协议，开箱即用填入密钥即可）

- **内置供应商对话框 Tab 切换**：内置供应商弹窗新增「<常规 / 视觉生成>」选项卡，可分别浏览与应用两类预设

### 🐞修复

- **Windows 剪贴板复制偶发** **`os error 1418`**：修复复制图片到剪贴板偶发报「线程没有打开的剪贴板」错误——Windows 平台绕开 arboard 直接调 Win32 API（OLE STA 初始化 + message-only 窗口作为剪贴板 owner + 消息泵重试循环），非 Windows 沿用原路径

- **全局布局高度溢出**：修复 AppLayout 容器 `h-screen` 导致所有页面底部 36px 被裁切、视觉生成页参数面板与画布底部被推出窗口外的问题，改用 `h-full` 并直接测量内容 wrapper 实测高度支撑滚动容器

### 🔄变更

- **移除工作区（Workspace）模块**：彻底下线工作区模块全部前后端实现（数据库表、路由、UI 组件、hooks 等）与相关设计文档，涉及配置档案生成能力一并清理

- **视觉生成供应商运行时隔离**：供应商新增视觉生成标识（`providers.is_media_generation`），网关转发 / 虚拟供应商逻辑按协议族识别并排除此类供应商——不参与 /v1/models 暴露与虚拟路由；虚拟供应商候选路由同样过滤视觉生成供应商

## [0.3.2] - 2026-09-02

> [!IMPORTANT]
> 数据库结构变更：模型配置表新增工具选择策略与请求参数字段，升级后首次启动自动完成迁移。

### 🚀新增

- **聊天「检测题目」工具**：聊天工具栏新增检测题目入口，弹窗表格展示内置 21 道检测题（题目 + 参考答案），支持一键将题目发送到当前会话（仅发送题目、不含答案），模型回答是否正确由人工对照答案列核对

- **聊天思考强度子面板**：模型下拉浮层改为「模型列表 + 思考强度面板」组合——左列支持本地搜索过滤、悬浮展示模型完整 ID；选中模型后右侧自动展示思考强度操作面板（开启思考开关、力度选项、自定义力度输入）；选项优先读取模型配置的推荐枚举，模型配置声明思考默认值（如 `{"type":"enabled","effort":"high"}`）时切换模型自动开启并套用对应力度

- **模型请求参数配置**：模型编辑弹窗新增工具选择策略、回复数量 n、停止序列、随机种子、流式返回 usage 等配置项，随模型配置持久化

- **聊天请求推理力度注入**：发送消息可携带推理力度（reasoning\_effort），按协议注入请求体（Chat → `reasoning_effort`，Responses → `reasoning.effort`）

- **下拉选定值对齐方式**：Select 触发器新增选定值水平对齐属性（靠左 / 居中 / 靠右），默认靠左

- **内置模型 SenseNova 6.8 Flash Lite**：内置模型库新增 SenseNova 6.8 Flash Lite（OpenAI 兼容协议，含推荐思考力度与工具选择枚举）

### 🐞修复

### 🔄变更

- **聊天工具栏轻量化**：SSE/HTTP 传输模式与协议下拉改用无边框、无胶囊的轻量化下拉组件，工具栏更紧凑

- **思考控件整合**：移除工具栏独立的思考开关与推理力度下拉，交互整合进模型选择浮层的思考强度子面板

- **迁移版本合并**：原 V009（工具选择策略）与 V010（请求参数）迁移合并为单个 V009，schema 版本号相应调整为 9

## [0.3.1] - 2026-09-01

### 🚀新增

- **社区图片查看缩放控制**：图片放大弹窗底部新增半透明按钮组（缩小 / 重置 / 放大，0.25 步进，0.5\~4 倍），实时显示当前缩放比例；按钮悬浮于图片上层，图片放大至全屏后仍可继续操作

- **网关监听端口占用帮助引导**：网关设置「监听端口」旁新增帮助弹层，按当前平台展示「查找占用进程 → 结束进程」命令（Windows / macOS / Linux），包含 Windows 系统保留端口时重启 winnat 服务的处理方式，命令支持一键复制

### 🐞修复

- **社区刷新同步个人资料**：修复社区右上角刷新仅重拉帖子列表、个人栏统计（帖子 / 回复 / 积分 / 签到）与服务器不一致的问题，刷新时一并同步最新个人资料

- **仪表盘网关启动失败排查弹窗**：修复仪表盘网关启动开关在端口被占用 / 权限不足 / 地址不可用时仅提示失败、缺少排查指引的问题，与网关页一致弹出端口占用帮助弹窗
...
> [!IMPORTANT]
> 社区服务端新增账号体系并升级会话鉴权：旧版本客户端将无法访问社区，请升级到本版本；升级后首次进入社区需重新登录。

### 🚀新增

- **社区账号体系**：支持匿名登录（本机机器码身份）与用户名密码登录两种模式；匿名用户可设置「唯一用户名 + 密码」绑定为账号，之后可在任意设备登录同一身份（帖子 / 回复 / 积分原样保留）

- **社区注册 / 登录**：注册即创建与设备解耦的独立账号，重装系统或更换设备不丢失；登录后自动获得会话凭证，无需重复输入密码

- **账号规则约束**：用户名须以英文字母开头（4\~32 位字母 / 数字、区分大小写），密码至少 8 位；注册与绑定时服务端与客户端双重校验

- **会话鉴权机制**：社区业务接口改用服务端签发的会话凭证（60 天有效）鉴权，不再依赖随源码分发的固定密钥，杜绝固定密钥泄露导致的身份伪装风险；凭证过期或失效后自动回到登录页重新登录

### 🐞修复

- **修复固定密钥泄露导致的身份冒充风险**：社区服务端弃用随源码分发的固定访问密钥，旧版本客户端请求将被拦截并提示升级到最新版本

### 🔄变更

- **社区入口改版**：进入社区由「一键开启」改为登录 / 注册 / 匿名进入三种入口；未登录或会话失效时展示登录卡

- **个人栏登录管理**：个人资料卡新增登录模式徽章（匿名 / 账号）；匿名用户可绑定用户名密码（一键升级账号），账号用户可退出登录，退出后需重新登录
...
- **帖子打赏**：帖子详情页新增「打赏」按钮（非作者可见），弹窗支持快捷金额档位（1 / 6 / 18 / 66 积分）与自定义金额（1\~66 积分），实时展示当前积分并在余额不足时禁止确认

- **打赏积分流转与通知**：打赏金额实时从打赏者扣除并等额计入帖子作者积分，双向写入积分流水（可追溯）；打赏同步为作者生成「被打赏」消息通知，点击可跳转到对应帖子

- **打赏信息折叠区域**：存在打赏的帖子正文下方展示默认折叠的「打赏信息」区域（共打赏积分与人数），展开后实名展示打赏人列表（头像 / 昵称 / 金额 / 时间，分页加载）；该信息不随分享直链展示

- **打赏防刷限制**：每个用户每天最多打赏 3 次；不能打赏自己发布的帖子，也不能打赏同一设备 / 同一网络（发帖 IP 相同）发布的帖子；每人每帖仅可打赏一次且不可撤销；打赏操作应用业务限流

- **打赏级联清理**：帖子被删除时，其全部打赏记录一并清理
...
- **load\_balance 策略实现**：新增 `RouteSelector` trait 与 `FallbackSelector` / `LoadBalanceSelector` / `OnAllSelector` 三个实现；`load_balance` 策略按 `weight` 字段加权随机选择 1 条健康路由；`on_all` 暂降级为 fallback 顺序尝试（并发实现留待后续迭代）；路由表单在 load\_balance 策略下显示权重输入框
...
- **DeepSeek 思考修复（网关侧）**：网关设置新增「DeepSeek 思考修复」开关，解决 DeepSeek V4 思考模式下多轮对话报错（上游要求 assistant 消息必须携带 `reasoning_content`，但模型有时仅返回 tool\_calls 而不产生该字段，导致下一轮请求被 400 拒绝）
...
- **自研 logger 请求头展示（去敏）**：网关（inbound 入站请求头）与供应商 API（outbound 出站请求头，缺失时回退到入站头）日志展示请求头 JSON（敏感头值替换为 `***`），位于「模型 ID」下方一行，随导出写入 CSV / JSON
  ...
  - `UpstreamClient::execute` 签名改为 `&mut UpstreamContext`，各 Client（openai\_chat / anthropic / openai\_responses / websocket）在发送前对真实出站请求头做去敏快照写入 `UpstreamContext.request_headers_json`，供转发（provider-api）日志展示
...
- **日志分段大小写入加固**（`SizeAwareFileAppender`）：写入超限时不只判断滚动，改为按 `max_size` 拆分 buffer——先写满当前文件、滚动到新分段后继续写剩余部分，保证每个分段不超过 `max_size`（修复单块大写入出现 \~40MB 分段的隐患）；使用 `create_new`（O\_EXCL）独占创建分段并跳过已被占用的序号，从根上避免多进程 / 多次重启把同一分段追加撑大或序号复用
...
- **Grok Build 字段解析增强**：金额/对象/百分比字段候选键兼容 camelCase / snake\_case（`creditUsagePercent` / `credit_usage_percent`、`currentPeriod` / `current_period`、`productUsage` / `product_usage`、`monthlyLimit` / `monthly_limit` 等）；新增「月度用量百分比」指标（`used / monthlyLimit × 100`，仅额度上限 > 0 时输出）
