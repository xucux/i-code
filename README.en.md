...
<p align="center">
  <a href="./GUIDE.en.md">User Guide</a> •
...
- **Virtual Providers**: Hide underlying real providers behind a fixed model ID (`{alias}/{model_id}`) with prioritized candidate routes and automatic failover — clients stay unchanged when providers are added, removed, or unavailable.
...
- **Media Generation**: Built-in vision generation workspace and gallery that connects directly to image-generation providers — text-to-image, local artifact storage, generation history and usage stats included.
...
- **Lightweight Community**: In-app community module (posts / replies / points / shareable links) with Markdown rendering — exchange ideas and curate content without leaving the app.
